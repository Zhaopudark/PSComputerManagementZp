...
#>
    ...
    if (!$target_exist){
        if(!$source_exist){
            ...
            # 1           | non-existing                      | existing-simple-file      | pass(do nothing)
            Write-Log  "[Do nothing] The $Target is non-existing and the $Source is a simple file already."
        }elseif($source_simple_directory){
            # 1           | non-existing                      | existing-simple-directory | pass(do nothing)
            Write-Log  "[Do nothing] The $Target is non-existing and the $Source is a simple directory already."
        ...
        if(!$source_exist){
            ...
            Write-Log "Remove-Item -Path $Target -Force"
        ...
        if(!$source_exist){
            ...
            Write-Log "Remove-Item -Path $Target -Recurse -Force"
        ...
        if(!$source_exist){
            ...
            # 1           | existing-file-symbolic-link       | existing-simple-file      | del target
            Write-Log  "Remove-Item $Target -Force"
