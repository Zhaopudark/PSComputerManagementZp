...
if (Test-Path -LiteralPath "$($ModuleInfo.BuildPath)\$($ModuleInfo.ModuleVersion)"){
    $content = Get-ChildItem "$($ModuleInfo.BuildPath)\$($ModuleInfo.ModuleVersion)" -Exclude $ModuleInfo.LogDir
    Remove-Item $content -Force -Recurse
}else{
    New-Item -Path "$($ModuleInfo.BuildPath)\$($ModuleInfo.ModuleVersion)" -ItemType Directory
    New-Item -Path "$($ModuleInfo.BuildPath)\$($ModuleInfo.ModuleVersion)\$($ModuleInfo.LogDir)" -ItemType Directory
}

foreach ($item in (Get-ChildItem $ModuleInfo.BuildPath -Exclude $ModuleInfo.ModuleVersion)){
    if (Test-Path -LiteralPath "$($item.FullName)\$($ModuleInfo.LogDir)"){
        Copy-Item -Path "$($item.FullName)\$($ModuleInfo.LogDir)\*" -Destination "$($ModuleInfo.BuildPath)\$($ModuleInfo.ModuleVersion)\$($ModuleInfo.LogDir)" -Recurse -Force
    }
