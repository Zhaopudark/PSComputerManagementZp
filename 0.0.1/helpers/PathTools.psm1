...
function local:Assert-FileSystemAuthorized{
    ...
    elseif (((Get-Qualifier $Path) -eq (Get-Qualifier $global:Home)) -and !(Test-IsInInHome $Path)){
        throw "If `$Path: '$Path' is in SystemDisk, it sholuld be or in `$Home: $Home."
    }
}


function local:Reset-PathAttributes{
    <#
    .Description
    ...
    try{
        ...
        Write-Output "Reset-PathAttributes Exception: $PSItem"
...
function local:Get-PathType{
    ...
    try{      
        ...
        if ([bool]($Attributes -band [IO.FileAttributes]::Directory)){
            ...
            elseif (($Attributes -band [IO.FileAttributes]::ReparsePoint) -and ($Linktype -eq "SymbolicLink")) {
                return "$header\SymbolicLinkDirectory"
            }
            elseif (($Attributes -band [IO.FileAttributes]::ReparsePoint) -and ($Linktype -eq "Junction")) {
...
function local:Set-OriginalAcl{
    ...
    Need function Reset-PathAttributes
